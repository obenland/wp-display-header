 ...
 *    underlying post-meta storage `wpdh_header_image`, which is what
 *    the meta box submit handler writes) actually overrides
...
test.describe( 'WP Display Header', () => {
	test( 'renders the Header meta box on the post edit screen', async ( {
		...
		const postId = wp( 'post create --post_type=post --post_status=publish --post_title="Header meta box test" --porcelain' );
		...
		await page.goto( `/wp-admin/post.php?post=${ postId }&action=edit&classic-editor` );
	...
	test( 'overrides theme_mod_header_image for a post when post meta is set', async ( {
		page,
	} ) => {
		// Create a post and set the per-post header URL via wp-cli — the
		// same `wpdh_header_image` post meta the meta box submit handler
		// writes.
		const postId = wp(
			'post create --post_type=post --post_status=publish --post_title="Override Header" --porcelain'
		);

		const overrideUrl = 'https://example.com/wp-content/uploads/test-header.jpg';
		wp( `post meta update ${ postId } wpdh_header_image '${ overrideUrl }'` );

		// Apply the filter directly via wp eval so we don't depend on any
		// theme-specific markup. The plugin's `theme_mod_header_image`
		// filter callback queries the current post's meta on a singular
		// view; `wp eval` inside a `--url=<post_permalink>` context
		// reproduces that environment.
		const permalink = wp( `post url ${ postId }` );
		const filtered = wp(
			`--url='${ permalink }' eval "echo apply_filters( 'theme_mod_header_image', 'default-header.jpg' );"`
		);

		expect( filtered ).toBe( overrideUrl );
	} );

	test( 'leaves the default theme header in place when no override is set', async ( {
		page,
	} ) => {
		const postId = wp(
			'post create --post_type=post --post_status=publish --post_title="No Header Override" --porcelain'
		);
